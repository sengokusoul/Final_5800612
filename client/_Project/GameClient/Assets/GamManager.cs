...
*/
	...
	public void Login(){
		getName = name.text;
		getPass = pass.text;
		string Url = "http://ec2-13-126-252-100.ap-south-1.compute.amazonaws.com:8081/userpass/"+getName+"/"+getPass;
		HttpWebRequest request = (HttpWebRequest)WebRequest.Create(Url);
		HttpWebResponse response = (HttpWebResponse)request.GetResponse();
		Stream stream = response.GetResponseStream();
		string responseBody = new StreamReader(stream).ReadToEnd();
	...
	public void SubmitRegister(){
		if (name.text != "" & pass.text != "") {
			getName = name.text;
			getPass = pass.text;
			string Url = "http://ec2-13-126-252-100.ap-south-1.compute.amazonaws.com:8081/user/add/user?Name=" + getName + "&Pasword=" + getPass;
			HttpWebRequest request = (HttpWebRequest)WebRequest.Create (Url);
			HttpWebResponse response = (HttpWebResponse)request.GetResponse ();
			Stream stream = response.GetResponseStream ();
			string responseBody = new StreamReader (stream).ReadToEnd ();
